...
Elements that animate on scroll carry the class `.reveal`. The Intersection Observer adds `.visible` to trigger the animation. Skill bar widths are stored in a `data-width="XX"` attribute (numeric, no `%`) on each `.skill-bar-fill` element. The IntersectionObserver reads `dataset.width` and sets `style.width` directly, which triggers the CSS `width` transition.
...
In the Skills section, each `.skill-bar-track` contains a `.skill-bar-fill` element with `data-width="XX"` (numeric value, no `%`). Update that attribute. The `aria-valuenow` attribute on the same element should also be kept in sync. The IntersectionObserver sets `style.width` from this value when the section enters the viewport.
